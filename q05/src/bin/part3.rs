...
fn process(input: &str) -> String {
    ...
    loop {
        ...
        let shout = shout.parse::<u64>().unwrap();

        if shout > max_shout {
            max_shout = shout;
            println!("max_shout: {max_shout} round: {}", i + 1);
        }

        if !states.insert(dance_columns.clone()) {
            return max_shout.to_string();
        }
